...
if __name__ == '__main__':
    ...
    for i in range(len(test_keys)):
        ...
        plt.imshow(test_img)
        plt.savefig('images_4train/input_test_'+str(i)+'.png')
        plt.close()
    ...
    for i in range(len(train_keys)):
        ...
        plt.savefig('images_4train/train_'+str(i)+'.png')
        plt.close()

    h = hopfield(train_images[[1,3,4,8]], test_images, theta=0, nprocess=5000, storkey=False)
    ...
    for i in range(len(retrain_img)):
        ...
        plt.imshow(img)
        plt.savefig('images_4train/retrain_'+str(i)+'.png')
        plt.close()
    ...
    for i in range(len(test_keys)):
        ...
        plt.imshow(test_img)
        plt.savefig('images_4train/test_'+str(i)+'.png')
        plt.close()

        plt.imshow(proc_img)
        plt.savefig('images_4train/proc_'+str(i)+'.png')
        plt.close()